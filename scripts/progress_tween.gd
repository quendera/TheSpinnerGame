...
func finish_hints():
	...
	interpolate_method($"../hex_progress","set_shape",float($"../Spawner".sw-1)/($"../Spawner".accum_points.size()-1),float($"../Spawner".sw)/($"../Spawner".accum_points.size()-1),global.move_time_new,$"../action_tween".transition,$"../action_tween".ease_direction)
...
func slide_hints(new):
	...
	interpolate_property($"../hex_subwave","scale",Vector2(1,1)*norm*sqrt(1-sw_score),Vector2(1,1)*norm*sqrt(max(0,1-sw_score-new)),global.move_time_new,$"../action_tween".transition,$"../action_tween".ease_direction)
