...
<details><summary>Sample JSON from db (click to expand) </summary> 
...
```
<details><summary>Click to expand pretty print: </summary> 
 
Break down first by type (lists first, everything else second), category (`mo_`, `sw_`, etc.), and size (if list): 
